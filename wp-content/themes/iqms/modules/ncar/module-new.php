			...
			if ( $post_id ) {
				...
				if($satisfactory !== ''){
					...
					if(get_option('notification_'.$owner)){
						...
						$options[] = 'The '.$ncar_no_new.' you raised has already been responded by.';
						update_option( 'notification_'.$owner,  $options);
					} else {
						add_option( 'notification_'.$owner,  ['The '.$ncar_no_new.' you raised has already been responded by.']);
					}
				}
